...
cell_t *mk_string(SchemeY *s, char *_string) {
  cell_t *c = syM_alloc(s);
  set_string(c, syM_strdup(s, _string));
  return c;
}

cell_t *mk_symbol(SchemeY *s, char *_symbol) {
  cell_t *c = syM_alloc(s);
  set_symbol(c, syM_strdup(s, _symbol));
  return c;
}
...
vector_t *mk_vector_t(SchemeY *s, size_t _size) {
  ...
  v->_items = syM_calloc(s, _size, sizeof(cell_t));
  v->_len = 0;
  v->_size = _size;
  return v;
}
...
cell_t *mk_vector(SchemeY *s, size_t _size) {
  cell_t *c = syM_alloc(s);
  set_vector(c, mk_vector_t(s, _size));
  return c;
}

cell_t *mk_table(SchemeY *s, size_t _size) {
  cell_t *c = syM_alloc(s);
  set_table(c, mk_vector_t(s, _size));
  return c;
}
